    ...
    try {
        for (const [itemName, count] of Object.entries(requiredItems)) {
            ...
            if (amountToWithdraw > 0) {
                ...
                if (itemToWithdraw) {
                    const actualAmount = Math.min(amountToWithdraw, itemToWithdraw.count, 64); // Limit to a max of 64
...
bot.on('end', reason => log(`Disconnected: ${reason}`));                    log(`Could not find ${itemName} in chest. Will not be able to build this block.`);
                }
                await sleep(1000);
            }
        }
